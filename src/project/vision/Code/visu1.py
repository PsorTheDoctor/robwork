...
for index, true_index in enumerate(indices, 1):
    errors = []
    for iteration in range(0, 5):
        estimated_pose = np.loadtxt(
            settings.results_folder + 'estimate_' + str(true_index) + '_' + str(noise_sigma) + '_' + str(iteration) + '.txt'
        )
...
index_labels = ["Scene " + str(i) for i in indices]
indexes = np.arange(1, len(indices)+1)
